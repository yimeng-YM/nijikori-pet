...
description: 需要真实浏览器/CDP 才接管——登录后操作、动态或 JS 渲染页面、被反爬站点（小红书/微博/公众号/推特）、像人一样在浏览器里导航点击、网页媒体与视频帧提取。轻量公开搜索仍用桌宠自带 web_search，不触发本技能。
triggers:
  - 小红书
  - 微博
  - 公众号
...
> - **执行确认框**：桌宠的 `run_command`/`run_command_capture` 默认会弹窗请主人确认（Confirm Before Command）。做连续自动化前请让主人在配置里关闭该确认，否则每次命令都要点一次。
> - **WebFetch/WebSearch 替代**：本 skill 正文提到 WebFetch/WebSearch，桌宠没有同名工具。轻量搜索直接用桌宠自带 `web_search`；抓取已知 URL 的文本用 `curl.exe` + `run_command_capture`；需要登录态或动态内容再走 CDP。
> - **平台风险**：浏览器自动化操作社交平台（小红书/微博/公众号）有账号限流/封禁风险，正文要求用小号并已内置防护，继续操作即视为接受。
> - **子 Agent 并行**：本 skill 的「分治子 Agent」在桌宠里没有对应子 Agent 机制（只有 `delegate_to_harness`，跑 dsh）。请改为**串行**处理多目标，或在对话里明确分批。
...
3. 若 tab 在 /dashboard/log，用 POST http://localhost:3456/navigate?target=ID，body 为 https://api.kourichat.com/dashboard。
...
2. 侧边栏"Token 套餐"菜单是 span 元素，querySelector 用 :contains 无效，需用 JS 遍历匹配：
   `[...document.querySelectorAll("span")].find(e=>e.childElementCount===0 && e.textContent.indexOf("Token")>=0).click()`
3. 页面信息包含：当前生效套餐档位、抵扣倍率、权益周期、今日/本周/本期累计额度与用量百分比
4. 套餐额度每日0:00、每周一0:00重置；用完后自动切换按量计费